# pipeline-agent-isolation Specification

## Purpose

Define how a daemon-spawned pipeline agent subprocess is isolated from the orchestrating daemon by running with its own ephemeral `NM_HOME`, so that any `no-mistakes` CLI command the agent runs resolves to a separate daemon socket and pid and cannot stop, restart, or cancel the orchestrating run.
