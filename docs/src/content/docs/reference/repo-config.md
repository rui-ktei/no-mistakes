...
The daemon also reads `document.instructions`, `review.path_instructions`, `disable_project_settings`, `no_ci`, `ci.rerun_transient`, and `propose_commands` only from that trusted copy.
...
When empty, the agent detects and runs the smallest relevant tests itself (and is instructed never to run the complete repository suite), and - unless [`propose_commands`](#propose_commands) is disabled - proposes the canonical command it used as an edit to this file (see [propose_commands](#propose_commands)).
