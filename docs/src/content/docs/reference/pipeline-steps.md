...
- When `ticket_prefix_pattern` is configured and the branch name matches it, the PR title is instead prefixed with the matched work-item id (e.g. `WEB-12345: description`), stripping any conventional `type(scope): ` prefix so the title is not double-prefixed; a branch with no match falls back to conventional-commit formatting
