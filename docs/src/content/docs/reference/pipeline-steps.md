...
- When `ticket_prefix_pattern` is configured, the gate resolves a work-item id from the branch name, then the agent-generated title, then the first non-gate author commit subject on the branch (oldest first); when a match is found from any source the PR title is prefixed with it (e.g. `WEB-12345: description`), stripping any conventional `type(scope): ` prefix so the title is not double-prefixed; when no source carries a match the title uses conventional-commit formatting. The fallback title is prefixed the same way.
...
- PR body leads with a `## Proposed Commands` callout when the branch's `.no-mistakes.yaml` carries discovered `commands.{test,lint,format}` proposals not yet merged to the default branch (see [`propose_commands`](/no-mistakes/reference/repo-config/#propose_commands)); it is placed first so it survives body truncation, and notes that the proposals stay inert until the PR merges to the default branch
...
- The [`ci_timeout` reference](/no-mistakes/reference/global-config/#ci_timeout) owns idle re-arming (each upstream integration-branch advance re-arms the timer), unlimited monitoring, and fail-closed reconciliation while that gate is parked
...
- If the integration branch moves after `checks-passed`, keeps watching the same PR; a clean behind PR needs no action, while an actual GitHub, GitLab, or Azure DevOps merge conflict is auto-fixed by rebasing onto the base and re-pushing through the force-push safety guard
...
- On GitHub, GitLab, or Azure DevOps merge conflict: asks the agent to rebase onto the latest integration-branch tip and make the smallest correct root-cause fix for the conflicts, using user intent when available
