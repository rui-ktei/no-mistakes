...
func waitForDaemonStop(p *paths.Paths, instance daemonInstance) error {
	...
	for time.Now().Before(deadline) {
		...
		// A stop is only real once the daemon process is gone, because only
		// process exit releases the NM_HOME singleton lock. Neither signal
		// proves that alone: a health check that merely errors is inconclusive
		// (a daemon that died without cleanup leaves its socket behind), and a
		// stale-looking pid file cannot overrule a daemon that still answers.
		// So liveness must not be affirmed AND the process must be provably
		// gone - by the captured instance, or by the recorded pid.
		if !alive {
			if err == nil {
				if exited, exitErr := instance.exited(); exitErr == nil && exited {
					cleanupDaemonArtifacts(p)
