...
%s%s%s`, branch, baseSHA, sctx.Run.HeadSHA, sctx.IntegrationBranch(), conventional.ReleaseTypeRule, diffStat, finalDiff, userIntentPromptSection(sctx), executionContextPromptSection())
...
func assemblePRBody(sctx *pipeline.StepContext, whatChanged, riskLine, testingMD, pipelineMD string, bodyLimit int) string {
	...
	full := withProposedCommandsNote(prependIntentSection(sections, sctx), sctx)
	if bodyLimit <= 0 || scm.PRBodyLen(full) <= bodyLimit {
		return full
	}
	if testingMD != "" {
		sections = appendGeneratedSections(whatChanged, riskLine, "", pipelineMD)
		core := withProposedCommandsNote(prependIntentSection(sections, sctx), sctx)
...
func buildPRBody(body, riskLine, testingMD, pipelineMD string, sctx *pipeline.StepContext) string {
	return withProposedCommandsNote(buildPRBodySections(body, riskLine, testingMD, pipelineMD, sctx), sctx)
}
...
func buildPRBodySections(body, riskLine, testingMD, pipelineMD string, sctx *pipeline.StepContext) string {
	body = stripGeneratedSections(body)
