...
func (d *DB) InsertRun(repoID, branch, headSHA, baseSHA string) (*Run, error) {
	return d.InsertRunWithBaseAndIntent(repoID, branch, headSHA, baseSHA, "", nil)
}
...
func (d *DB) InsertRunWithBase(repoID, branch, headSHA, baseSHA, baseBranch string) (*Run, error) {
	return d.InsertRunWithBaseAndIntent(repoID, branch, headSHA, baseSHA, baseBranch, nil)
}

func (d *DB) InsertRunWithIntent(repoID, branch, headSHA, baseSHA string, intent *RunIntent) (*Run, error) {
	return d.InsertRunWithBaseAndIntent(repoID, branch, headSHA, baseSHA, "", intent)
}

// InsertRunWithBaseAndIntent is the single insert path: it stamps the per-run
// base-branch override and the run's intent provenance in the same statement,
// so a rerun's inherited intent source is never overwritten after the fact.
func (d *DB) InsertRunWithBaseAndIntent(repoID, branch, headSHA, baseSHA, baseBranch string, intent *RunIntent) (*Run, error) {
	ts := now()
	r := &Run{
		ID:               newID(),
		RepoID:           repoID,
	...
	_, err := d.sql.Exec(
		`INSERT INTO runs (id, repo_id, branch, head_sha, base_sha, base_branch, submitted_head_sha, status, pr_state, intent, intent_source, intent_session_id, intent_score, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'none', ?, ?, ?, ?, ?, ?)`,
		r.ID, r.RepoID, r.Branch, r.HeadSHA, r.BaseSHA, r.BaseBranch, headSHA, r.Status, r.Intent, r.IntentSource, r.IntentSessionID, r.IntentScore, r.CreatedAt, r.UpdatedAt,
	)
	if err != nil {
