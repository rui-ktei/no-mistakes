...
func newStShortcutCmd() *cobra.Command {
	return newRunStatusCommand("st", "Show the active or most recent pipeline run in detail (shortcut for axi status)", "st")
}
...
func newLgShortcutCmd() *cobra.Command {
	return newRunLogsCommand("lg", "Show the log output of one pipeline step (shortcut for axi logs)", "lg", []string{"logs"})
}
