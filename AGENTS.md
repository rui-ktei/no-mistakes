...
- Every daemon-spawned pipeline agent runs with an isolated, per-run `NM_HOME` (`<NM_HOME>/agent-homes/<runID>`, created before the agent, removed on run end): without it a `no-mistakes` CLI the agent runs (the test step's evidence agent when self-gating) reenters the orchestrating daemon's socket/pid/gate and tears it down mid-run (`daemon crashed during execution`). Applied at one chokepoint via `agent.Options.EnvOverrides` (keyed `NM_HOME`) in `agentEnv` (`internal/agent/env.go`) and threaded through `startServerWithPort` for managed-server backends; set in `internal/daemon/manager.go` `startRun`, torn down in the run goroutine's teardown. Never read the orchestrator's `NM_HOME` from inside an agent backend. Regressions: `TestAgent_EnvOverrideIsolatesNMHome`, `TestPushReceivedCreatesAndCleansUpAgentHome`.
...
- When no daemon is live the classifier reads the registry through `db.OpenReadOnly`, which by design never migrates, so a freshly upgraded binary meets the *previous* version's schema. Its queries must therefore project only columns present since the initial schema: use `GetActiveRunIdentities`/`GetActiveStepIdentitiesByRun` (`internal/db/gatecontext_projection.go`), never the full-row `GetActiveRuns`/`GetStepsByRun`. Reading a migration-added column here breaks every mutation command after an upgrade (`make install`'s `daemon stop` died on `no such column: submitted_head_sha`). Regressions: `TestGetActiveRunIdentities_ReadsPreMigrationSchema`, `TestGetActiveStepIdentitiesByRun_*`.
...
- Deciding "the daemon has stopped" takes two signals, because neither is sufficient alone: an unreachable health check is inconclusive (a daemon that dies without cleanup leaves its socket file, so probing it errors instead of reporting a clean not-alive), while a stale-looking pid file cannot overrule a daemon that still answers. `waitForDaemonStop` therefore concludes a stop only when the health check does not affirm liveness **and** `recordedDaemonProcessGone` proves the recorded process is gone; a daemon answering its health check is never cleaned up whatever its pid file says. Getting this wrong made `daemon stop` fail permanently against a dead daemon's leftovers (`inspect daemon pid <n>: exit status 1`, blocking `make install`). Regressions: `TestWaitForDaemonStop_TreatsDeadRecordedPIDAsStopped`, `TestWaitForDaemonStop_StillKillsLiveUnresponsiveDaemon`, `TestWaitForDaemonStopKeepsArtifactsWhenKillFails`, `TestWaitForDaemonStopDoesNotTreatHealthCheckErrorsAsStopped`.
...
- Never run the `e2e`-tagged suite from inside a gated no-mistakes pipeline run (e.g. while validating with `no-mistakes axi run`): each e2e test spawns its own daemon, and running that concurrently with the live pipeline daemon starves the nested daemon's startup handshake past the harness timeouts, so `init` appears to hang. This is resource contention, not a regression. Inside a gated run, rely on the baseline `go test -race ./...` (which excludes the tag) plus focused temp-repo tests; run `make e2e` separately, outside the pipeline.
...
- `document.instructions` (the repo's documentation placement policy), `disable_project_settings` (the gate-agent project-instruction opt-out), and `propose_commands` are also trusted-only: a pushed branch must not weaken either boundary or flip the proposal switch. When the opt-out is enabled, only adapters with verified effective suppression may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`, `ticket_prefix_pattern`) are still read from the pushed branch.
...
- The **base branch override** (`repos.base_branch` via `init --base-branch`, or per run via `axi run --base`) moves only the integration target (rebase-onto branch, review/lint/document diff base, PR base/skip check), resolved through `pipeline.ResolveIntegrationBranch` (per-run override → `repo.base_branch` → `repo.default_branch` → `main`) and read by steps via `StepContext.IntegrationBranch()`. It MUST NOT move the trust root: `startRun` still resolves `trustedSHA` from `repo.DefaultBranch`, so a contributor cannot relocate the trusted-config source by setting a base branch. Regression: `TestTrustRootIgnoresBaseBranchOverride`.
...
**Base Branch Override (integration target)**

- `repos.base_branch` (DB column, additive `NOT NULL DEFAULT ''`) and `runs.base_branch` (per-run override) carry the integration branch. `init --base-branch` persists it (preserved on idempotent refresh like `fork_url`, via `gate.InitWithOptions`); `axi run --base` sets it for one run, carried through a `no-mistakes.base=` push option → `PushReceivedParams`/`RerunParams` → `startRun`/`HandleRerun`, and reused on rerun like `BaseSHA`.
- `init --base-branch` is validated against origin before it persists (`gate.validateBaseBranch` via `git.LsRemote refs/heads/<branch>`): a branch origin does not have is rejected upfront, since a persisted typo would retarget rebase/review/PR for every later run and fail confusingly on the missing ref. The per-run `axi run --base` is not pre-validated (it is transient, one run only). Regression: `TestInitBaseBranchValidatedAgainstOrigin`.
- All base consumers route through `StepContext.IntegrationBranch()`: `rebase.go` (rebase target + bundled-local-base and force-push-on-base checks), `review.go`/`lint.go`/`document.go`/`test.go`/`intent.go`/`common_fix.go` (diff base), `pr.go` (PR base + PR-skip check), and `ci.go`/`ci_fix.go` (PR-base tip for re-arm/rebase). When adding a new diff/rebase/PR consumer, read the base from `IntegrationBranch()`, never `Repo.DefaultBranch` directly. The force-push lease and bundled-local-base protections operate on the resolved integration branch (regression `TestRebaseStep_DetectsUnpushedLocalBaseBranchCommitsForOverride`).
