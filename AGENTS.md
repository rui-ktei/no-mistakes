...
- The **base branch override** (`repos.base_branch` via `init --base-branch`, or per run via `axi run --base`) moves only the integration target: the rebase-onto branch, the review/lint/document diff base, and the PR base/skip check, resolved through `pipeline.ResolveIntegrationBranch` (per-run override → `repo.base_branch` → `repo.default_branch` → `main`) and read by steps via `StepContext.IntegrationBranch()`. It MUST NOT move the trust root: `startRun` still resolves `trustedSHA` from `repo.DefaultBranch`, so `loadTrustedRepoConfig` keeps reading `commands`/`agent` from the auto-detected default branch. A contributor cannot relocate the trusted-config source by setting a base branch. Regression: `TestTrustRootIgnoresBaseBranchOverride`.
