...
- `document.instructions` (the repo's documentation placement policy), `disable_project_settings` (the gate-agent project-instruction opt-out), and `propose_commands` are also trusted-only: a pushed branch must not weaken either boundary or flip the proposal switch. When the opt-out is enabled, only adapters with verified effective suppression may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `intent`, `test`, `ticket_prefix_pattern`) are still read from the pushed branch.
